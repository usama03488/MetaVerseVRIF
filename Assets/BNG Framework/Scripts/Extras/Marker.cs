...
namespace BNG {
    public class Marker : GrabbableEvents {
        ...
        IEnumerator WriteRoutine() {
            while (true) {
                if (Physics.Raycast(RaycastStart.position, RaycastStart.up, out RaycastHit hit, RaycastLength, DrawingLayers, QueryTriggerInteraction.Ignore)) {
                    ...
                    {
                        photonView.RPC("SyncDraw", RpcTarget.All, drawStart, drawRotation, lineWidth, 255,0,0,0);
                    }
        ...
        [PunRPC]
        void SyncDraw(Vector3 position, Quaternion rotation, float lineWidth, int r, int g, int b, int a)
        {
            Color lineColor = new Color(r / 255f, g / 255f, b / 255f, a / 255f);
            InitDraw(position, rotation, lineWidth, lineColor);
        }
        ...
        Vector3 DrawPoint(Vector3 lastDrawPoint, Vector3 endPosition, float lineWidth, Color lineColor, Quaternion rotation) {
            ...
            if (IsNewDraw || LineRenderer == null) {
                //lastTransform = new GameObject().transform; original
                lastTransform = PhotonNetwork.Instantiate("DrawLine", endPosition, rotation).transform; 
        ...
        public void ClearBoard()
        {
            if (root != null)
            {
                // Destroy all child objects of the root
        ...
        public void OnClearButtonPressed()
        {
            ...
            {
                // Call ClearBoard on all clients
                photonView.RPC("ClearBoard", RpcTarget.All);
            }
            else
            {
                // If not using multiplayer, clear locally
                ClearBoard();
            }
        }
