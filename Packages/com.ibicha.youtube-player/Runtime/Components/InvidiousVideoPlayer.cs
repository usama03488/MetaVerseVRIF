...
namespace YoutubePlayer.Components
{
    ...
    public class InvidiousVideoPlayer : MonoBehaviour
    {
        ...
        public  string ExtractYouTubeVideoId(string url)
        {
            ...
            {
                Debug.LogError("It is a video Link");
                var queryStart = url.IndexOf("?") + 1;
                if (queryStart > 0)
                {
                    Debug.LogError("questarted");
                    ...
                    foreach (string param in queryParams)
                    {
                        if (param.StartsWith("v="))
                        {
                            ...
                            Debug.LogError("Video plaing" + VideoId);
                            OnPlayButtonPressed();
                ...
                {
                    Debug.LogError("Can not parse 2");
                }
            }
            ...
            {
                var idStart = url.IndexOf("youtu.be/") + "youtu.be/".Length;
                VideoId = url.Substring(idStart);
        ...
        /*    m_PlayingVideoId = VideoId;
            ...
            VideoPlayer.Play();*/
            Debug.LogError("Button pressed called");
            Debug.LogError("Video player" + VideoPlayer.name);
         await PlayVideoAsync();
            // Call the async method but without awaiting directly
            //  _ = PlayVideoAsync(); // Extension method to suppress warnings, or you can call it directly
        }
